...
//                            int yCoord = Integer.parseInt(messageArr[1].split("-")[1]);
...
//                            break;
//                        }
//                        // Scrapped this because our Rpi will send live updates
//                        // We no longer receive all instructions and then process them
//                        // Instead, we receive instructions and execute the GUI changes on the fly
//                        case ("DONE"): {
//                            int numInst = 1;
//                            // if number of instructions to execute is more than 1, update numInst
//                            if(messageArr.length > 1) numInst = Integer.parseInt(messageArr[1]);
//                            // if previously computed moveList is empty or smaller than the number of instructions to execute, break
//                            if (moveList.size() < numInst || moveList.size() == 0) break;
//                            // finalNumInst is the final number of instructions to execute
//                            int finalNumInst = numInst;

//                            // Starts a new thread to control the GUI robot movement (without stalling program)
//                            new Thread() {
//                                public void run() {
//                                    try {
//                                        // If robot is moving, pause the thread for 100 milliseconds (need to coordinate with STM)
//                                        while(MOVING) sleep(100);

//                                        // Perform initialisation (update directions, new movement list based on the subset of instructions)
//                                        MOVING = true;
//                                        String prevDir = "N";
...
//                                        // Executes the entire newMoveList
//                                        for (int i = 0; i < finalNumInst; i++) {
//                                            String[] args = newMoveList.get(0).replaceAll("\\(|\\)", "").split(",");

//                                            // Extracts the x-coord, y-coord, and direction of robot
//                                            int xCoord = (int) Double.parseDouble(args[0].trim());
//                                            int yCoord = (int) Double.parseDouble(args[1].trim());
//                                            String dir = "N";
//                                            switch (args[2].trim()) {
//                                                case ("0"):
//                                                    dir = "N";
...
//                                            // If current direction is same as previous, the motion is a forward or backward (much faster at 700ms)
//                                            if (dir.equals(prevDir)){
//                                                Thread.sleep(700);
//                                            }
//                                            // Otherwise, it involves a turn, and the motion is slower at 5000ms
//                                            else{
//                                                Thread.sleep(5000);
//                                            }

//                                            // Update the previous direction to the current direction (for subsequent iterations)
//                                            prevDir = dir;

//                                            // Set the robot coordinate and direction
//                                            arena.setRobot(xCoord, yCoord, dir);

//                                            // Send a reply message to Rpi to confirm the changes
...
//                            context.sendBroadcast(rcv_intent);
    ...
    public void setConnectBtn(){
        ...
        if (BluetoothService.getBtStatus() == BluetoothService.BluetoothStatus.CONNECTING) {
            ...
            bt.setBackgroundColor(getResources().getColor(R.color.purple_300));
        }
        else if (BluetoothService.getBtStatus() == BluetoothService.BluetoothStatus.DISCONNECTED) {
